...
///
/// Sets up the rustls crypto provider, tracing, DNS resolver, shared application state,
/// background tasks, CORS and admin routes, transparent and explicit proxy listeners,
/// and coordinates graceful shutdown and resource draining.
///
/// # Examples
///
/// ```no_run
/// // Run the application (binary entrypoint)
/// // $ cargo run --bin ssl_proxy
/// ```
#[tokio::main]
async fn main() {
    ...
    let _wg_relay = if config.wireguard.obfuscation_enabled {
        Some(
