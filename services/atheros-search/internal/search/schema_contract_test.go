...
func TestCanonicalTiDBSchemaMatchesQueryFacade(t *testing.T) {
	_, currentFile, _, ok := runtime.Caller(0)
	require.True(t, ok)
	root := filepath.Clean(filepath.Join(filepath.Dir(currentFile), "../../../.."))
	files := []string{
		"sql/tidb/atheros_search/01_tables/001_schema_manifest.sql",
		"sql/tidb/atheros_search/01_tables/002_search_documents.sql",
		"sql/tidb/atheros_search/01_tables/003_search_vectors.sql",
		"sql/tidb/atheros_search/01_tables/004_projection_state.sql",
		"sql/tidb/atheros_search/01_tables/005_graph_inventory_identity.sql",
		"sql/tidb/atheros_search/01_tables/006_query_feedback.sql",
	}
	combined := ""
