...
pub fn wallTimestampMs() ?i64 {
    var ts: std.posix.timespec = undefined;
    switch (std.posix.errno(std.posix.system.clock_gettime(.REALTIME, &ts))) {
        .SUCCESS => {
            const seconds = @as(i64, @intCast(ts.sec)) * std.time.ms_per_s;
            const millis = @divTrunc(ts.nsec, std.time.ns_per_ms);
    ...
    fn compareValue(self: *JsonComparer, a: std.json.Value, b: std.json.Value, diffs: *Diffs, path: *std.ArrayList([]const u8)) !void {
        ...
        switch (a) {
            ...
            .object => {
                ...
                var reverse_it = b.object.iterator();
                while (reverse_it.next()) |entry| {
                    const key = entry.key_ptr.*;
                    if (a.object.get(key) != null) continue;
