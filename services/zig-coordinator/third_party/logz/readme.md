...
pub const Config = struct {
    ...
    large_buffer_strategy: .create, // or .drop
};
```
...
pub fn main() !void {
    ...
    try logz.setup(io, allocator, .{
        ...
        .buffer_size = config.log.buffer_size,
...
* `logz_pool_empty` - counts the number of times the log pool was empty. Depending on the `pool_strategy` configuration, this either results in a logger being dynamically allocated, or a log message being dropped. Consider increasing `pool_size`.
* `logz_large_buffer_empty` - counts the number of times the large buffer pool was empty. Depending on the `large_buffer_strategy` configuration, this either results in a large buffer being dynamically allocated, or part of a log being dropped. Consider increasing `large_buffer_count`.
