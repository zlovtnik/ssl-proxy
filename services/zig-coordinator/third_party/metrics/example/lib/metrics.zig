...
const Metrics = struct {
    ...
    const Latency = m.HistogramVec(f64, LatencyLabel, &.{ 0.05, 0.10, 0.50, 1, 2.5, 5, 10 });
};
...
pub fn latency(labels: LatencyLabel, value: f64) !void {
    return metrics.latency.observe(labels, value);
}

pub fn initialize(allocator: Allocator, io: Io, comptime opts: m.RegistryOpts) !void {
    metrics = .{
