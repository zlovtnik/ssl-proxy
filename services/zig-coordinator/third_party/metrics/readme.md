...
Please see the example project. It demonstrates how a <a href="https://github.com/karlseguin/metrics.zig/blob/master/example/lib/metrics.zig">library developer</a>, and how an <a href="https://github.com/karlseguin/metrics.zig/blob/master/example/main.zig">application developer</a> can initialize and output them.
...
* `help: ?[]const u8 = null` - Used to generate the `# HELP $HELP` output line

The second option should have two fields:
...
The labels are strongly typed. Valid label types are: `ErrorSet`, `Enum`, `Type`, `Bool`, `Int` and `[]const u8`
...
#### `initializeNoop(T) T`
Creates an initializes metric `T` with `noop` implementation of every metric field. `T` should contain only metrics (`Counter`, `Gauge`, `Histogram` or their vectored variants) and primitive fields (int, bool, []const u8, enum, float). 
...
### CounterVec(T, L)
A `CounterVec(T, L)` is used for incrementing values with labels. `T` can be an unsigned integer or a float. `L` must be a struct where the field names and types will define the labels. Its two main methods are `incr(labels: L)` and `incrBy(labels: L, value: T)`. `incr(L)` is a short version of `incrBy(L, 1)`.

#### `init(allocator: Allocator, comptime name: []const, comptime opts: Opts, comptime ropts: RegistryOpts) !CounterVec(T, L)`
...
### GaugeVec(T, L)
A `GaugeVec(T, L)` is used for incrementing values with labels. `T` can be an integer or a float. `L` must be a struct where the field names and types will define the labels. Its main methods are `incr(labels: L)`, `incrBy(labels: L, value: T)` and `set(labels: L, value: T)`. `incr(L)` is a short version of `incrBy(L, 1)`.
...
### HistogramVec(T, L, []T)
A `HistogramVec(T, L, []T)` is used to track the size and frequency of events. `T` can be an unsigned integer or a float. `L` must be a struct where the field names and types will define the labels. Its main method is `observe(labels: L, value: T)`.

Observed valued will fall within one of the provided buckets, `[]T`. The buckets must be in ascending order. A final "infinite" bucket *should not* be provided.

#### `init(allocator: Allocator, comptime name: []const, comptime opts: Opts, comptime ropts: RegistryOpts) !HistogramVec(T, L, []T)`
Initializes the histogram. Name must be given at comptime. 

Opts is:
* `help: ?[]const` - optional help text to include in the prometheus output

#### `deinit(self: *HistogramVec(T, L, []T)) void`
Deallocates the histogram

#### `observe(self: *HistogramVec(T, L, []T), labels: L, value: T) !void`
Observes `value`, bucketing it based on the provided comptime buckets.

#### `remove(self: *HistogramVec(T, L, []T), labels: L) void`
Removes the labeled value from the histogram. Safe to call if `labels` is not an existing label.

#### `write(self: *HistogramVec(T, L, []T), writer: *std.io.Writer) !void`
Writes the histogram to `writer`.
