    ...
    return union(enum) {
        ...
        pub const Impl = struct {
            ...
            fn atomicIncrCallback(value: V, entry: *Value) void {
                _ = @atomicRmw(V, &entry.value, .Add, value, .monotonic);
            }
            ...
            fn atomicSetCallback(value: V, entry: *Value) void {
                @atomicStore(V, &entry.value, value, .monotonic);
            }
            ...
            pub fn write(self: *Impl, writer: *std.Io.Writer) !void {
                ...
                while (it.next()) |kv| {
                    ...
                    const value_ptr = kv.value_ptr;
                    try writer.writeAll(value_ptr.attributes);
                    try m.write(@atomicLoad(V, &value_ptr.value, .monotonic), writer);
