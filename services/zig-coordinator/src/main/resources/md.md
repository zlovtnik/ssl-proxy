# Migration Plan: Zig Coordinator → Java/Spring Boot/Apache Camel
...
### Phase 5: Deployment
19. `Dockerfile` — `eclipse-temurin:21-jre`-based slim image
...
Would you like me to proceed with this plan? If so, please toggle to ACT MODE — I'll start building Phase 1 (Gradle project + foundation).

There are two things I'd like your input on before starting:

1. **Package name**: I used `com.sslproxy.coordinator` — would you prefer something different?
2. **Wireless handlers thread safety**: The Zig runs all 7 wireless handlers sequentially each iteration (one message per consumer). With Camel Kafka, we could process them concurrently via separate consumer groups (since they're independent). Should we keep sequential processing (same as Zig) or go concurrent?