...
def _check_image_updater(updater: Document, relative: Path, errors: list[str]) -> None:
    write_back = _mapping(_path(updater, "spec", "writeBackConfig"))
    git = _mapping(write_back.get("gitConfig"))
    if write_back.get("method") != "git:secret:argocd/ssl-proxy-image-updater-git":
        errors.append(f"{relative}: missing 'method: git:secret:argocd/ssl-proxy-image-updater-git'")
    for field, expected, label in (("branch", "main", "branch: main"), ("writeBackTarget", "kustomization", "writeBackTarget: kustomization")):
        if git.get(field) != expected:
            errors.append(f"{relative}: missing {label!r}")
    if not isinstance(git.get("pullRequest"), Mapping):
        errors.append(f"{relative}: missing 'pullRequest:'")
    refs = _list(_path(updater, "spec", "applicationRefs"))
    by_pattern = {str(_mapping(reference).get("namePattern")): _mapping(reference) for reference in refs if isinstance(reference, Mapping)}
    for pattern in ("ssl-proxy-data-plane", "ssl-proxy-app-stack"):
        if pattern not in by_pattern:
            errors.append(f"{relative}: missing 'namePattern: {pattern}'")
    images = [image for reference in by_pattern.values() for image in _list(reference.get("images")) if isinstance(image, Mapping)]
    if not any(_mapping(image.get("commonUpdateSettings")).get("updateStrategy") == "digest" for image in images):
        errors.append(f"{relative}: missing 'updateStrategy: digest'")
    known = {str(image.get("imageName")) for image in images}
    for image in FIRST_PARTY_IMAGES:
        reference = f"192.168.1.221:5000/{image}:latest"
        if reference not in known:
            errors.append(f"{relative}: missing dev image mapping {reference!r}")
    if any("ssl-proxy-prod-" in str(reference.get("namePattern", "")) for reference in refs if isinstance(reference, Mapping)):
...
def check_repository(root: Path, executable: str) -> list[str]:
    ...
    updater_documents = _read_yaml_required(root, updater_relative, errors, "Image Updater manifest")
    if updater_documents is not None:
        updater = _find(updater_documents, "ImageUpdater", "ssl-proxy-dev")
        if len(updater) != 1:
            errors.append(f"{updater_relative}: required Image Updater manifest is missing")
        else:
            _check_image_updater(updater[0], updater_relative, errors)

    controller_relative = Path("cyber-stack/argocd/application-image-updater.yaml")
    controller_documents = _read_yaml_required(root, controller_relative, errors, "Image Updater controller Application")
    if controller_documents is not None:
        controller = _find(controller_documents, "Application", "ssl-proxy-image-updater")
        if len(controller) != 1:
            errors.append(f"{controller_relative}: required Image Updater controller Application is missing")
        else:
            source = _mapping(_path(controller[0], "spec", "source"))
            helm = _mapping(source.get("helm"))
            values = _mapping(helm.get("valuesObject"))
            requirements = (("project: ssl-proxy-image-updater", _path(controller[0], "spec", "project") == "ssl-proxy-image-updater"), ("repoURL: https://argoproj.github.io/argo-helm", source.get("repoURL") == "https://argoproj.github.io/argo-helm"), ("chart: argocd-image-updater", source.get("chart") == "argocd-image-updater"), ("targetRevision: 1.2.4", source.get("targetRevision") == "1.2.4"), ("createClusterRoles: false", _path(values, "createClusterRoles") is False), ("api_url: http://192.168.1.221:5000", any(_mapping(registry).get("api_url") == "http://192.168.1.221:5000" for registry in _list(_path(values, "config", "registries")))), ("prefix: 192.168.1.221:5000", any(_mapping(registry).get("prefix") == "192.168.1.221:5000" for registry in _list(_path(values, "config", "registries")))), ("insecure: true", any(_mapping(registry).get("insecure") is True for registry in _list(_path(values, "config", "registries")))), ("namespace: argocd", _path(controller[0], "spec", "destination", "namespace") == "argocd"), ("automated:", isinstance(_path(controller[0], "spec", "syncPolicy", "automated"), Mapping)), ("prune: true", _path(controller[0], "spec", "syncPolicy", "automated", "prune") is True), ("selfHeal: true", _path(controller[0], "spec", "syncPolicy", "automated", "selfHeal") is True), ("allowEmpty: false", _path(controller[0], "spec", "syncPolicy", "automated", "allowEmpty") is False))
            for label, valid in requirements:
                if not valid:
                    errors.append(f"{controller_relative}: missing {label!r}")
            if _path(values, "createClusterRoles") is True:
                errors.append(f"{controller_relative}: Image Updater must remain namespace scoped")

    project_relative = Path("cyber-stack/argocd/appproject-image-updater.yaml")
    project_documents = _read_yaml_required(root, project_relative, errors, "Image Updater AppProject")
    if project_documents is not None:
        projects = _find(project_documents, "AppProject", "ssl-proxy-image-updater")
        if len(projects) != 1:
            errors.append(f"{project_relative}: required Image Updater AppProject is missing")
        else:
            project = projects[0]
            required = (("name: ssl-proxy-image-updater", True), ("namespace: argocd", _metadata(project).get("namespace") == "argocd"), ("https://argoproj.github.io/argo-helm", "https://argoproj.github.io/argo-helm" in _list(_path(project, "spec", "sourceRepos"))), ("group: apiextensions.k8s.io", any(_mapping(entry).get("group") == "apiextensions.k8s.io" for entry in _list(_path(project, "spec", "clusterResourceWhitelist")))), ("kind: CustomResourceDefinition", any(_mapping(entry).get("kind") == "CustomResourceDefinition" for entry in _list(_path(project, "spec", "clusterResourceWhitelist")))))
            for label, valid in required:
                if not valid:
                    errors.append(f"{project_relative}: missing {label!r}")
            allowed = _list(_path(project, "spec", "namespaceResourceWhitelist")) + _list(
                _path(project, "spec", "clusterResourceWhitelist")
            )
            for forbidden in ("ClusterRole", "ClusterRoleBinding"):
                if any(_mapping(entry).get("kind") == forbidden for entry in allowed):
                    errors.append(f"{project_relative}: namespace-scoped updater must not allow {forbidden}")
