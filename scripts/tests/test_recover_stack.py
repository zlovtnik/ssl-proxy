...
spec:
  template:
    spec:
      ...
      containers:
        - name: schema
          image: {prefix}/postgres-runtime-schema@{PIN}
"""
    return {
        "bootstrap": bootstrap,
        "app-stack": app_stack,
...
class RecoverStackTest(unittest.TestCase):
    ...
    def test_healthy_aligned_stack_returns_zero(self) -> None:
        ...
        self.assertIn("ConfigMap/ssl-proxy-prod-postgres-endpoint: PRESENT", report)
    ...
    def test_warning_events_and_command_errors_are_sanitized(self) -> None:
        event_runner = FakeRunner(
            ...
            event_message=(
                ...
                "dsn=service:dsn-password@tcp(postgres:5432)/db "
    ...
    def test_full_report_precedes_nonzero_for_all_blocker_classes(self) -> None:
        ...
        self.assertEqual(1, returncode)
        self.assertIn("ConfigMap/ssl-proxy-prod-postgres-endpoint: MISSING", report)
