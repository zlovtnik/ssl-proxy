...
This threat model covers the current runtime described in [System Architecture](architecture.md), the PostgreSQL ownership model in [PostgreSQL manifests](../sql/postgres/), and the live operational guardrails in [Secret Management](secret-management.md). It is an engineering baseline, not a completed compliance assessment.
...
- Registry, Kubernetes, identity-provider, PostgreSQL and observability credentials
...
flowchart LR
    ...
    octopus --> postgres[PostgreSQL data boundary]
    octopus --> minio[MinIO data boundary]
    schema_migrator[Schema Migrator] --> postgres
    search[Atheros Search API and workers] --> postgres
    search --> minio
...
The proxy and sensor are privileged networking components but intentionally do not receive direct PostgreSQL credentials. Octopus, Atheros Search and Schema Migrator are the approved direct PostgreSQL clients, each with isolated accounts and domain-scoped grants. No other runtime path may establish direct database connections.

## Primary threats and controls

| Priority | Threat | Impact | Current controls | Remaining risk |
|---|---|---|---|---|
| P1 | Tampering or replay of `wireless.audit` or `sync.scan.request` | Duplicate ingestion, stale work, false evidence or incorrect projections | Event identity via schema-versioned event IDs; deduplication scope per topic/partition/offset with durable PostgreSQL evidence; offset-reset behavior documented per topic; producer/consumer compatibility checks enforced at contract boundary; locked topic names, schema-versioned events, durable dedupe, replay-safe offsets, and per-topic/partition/offset evidence | Operators with Redpanda and PostgreSQL control can still corrupt evidence if external immutability is not enforced |
...
| P1 | Compromised direct PostgreSQL client or over-broad grant | Cross-domain data modification, data exfiltration or projection corruption | Separate databases and accounts, verified TLS, table-level grant matrix, manifest checksum validation and fail-closed startup checks | Grant drift or misconfiguration can still enable accidental privilege creep |
...
| P1 | Exposed admin, PostgreSQL, Search API, Grafana or observability ports | Unauthorized control-plane access, data disclosure or cross-service visibility | Every public endpoint is enumerated with port, allowed source network, authentication method and responsible owner: Proxy admin (port 3002, loopback, API key, proxy owner); Search API (configurable port, cluster-internal, token auth, search owner); Search UI (same host as Search API, cluster-internal, token auth, search owner); Octopus health (cluster-internal, unauthenticated liveness / authenticated readiness, coordinator owner); Grafana (TLS NodePort 30000, trusted LAN only, admin credential, observability owner); Loki/Jaeger (cluster-internal, identity-provider auth, observability owner); all other PostgreSQL and registry surfaces remain internal-only. Search API and UI must fail closed when authentication is absent. | Firewall drift, split-horizon DNS mistakes, operator misrouting or accidental public exposure remain deployment risks |
...
- The Rust proxy, sync-plane crate and Atheros Sensor do not connect directly to PostgreSQL.
- Wireless persistence flows through Redpanda and the Octopus discovery pipeline; the sensor does not own database state.
- PostgreSQL is the only supported runtime database. It is allowed as a direct client for Octopus, Atheros Search and Schema Migrator; no other service may establish direct database connections.
...
- Public ingress is limited to the existing WireGuard UDP entrypoint and the route-free, IPv4-only Traefik TCP 80 edge. Traefik TCP 443 remains LAN-testable but is not WAN-forwarded. Phase one returns only Traefik's generic 404; admin, application, DB and observability surfaces remain internal except Grafana, which is a direct TLS NodePort on 30000 restricted by the node firewall to the trusted LAN and never router-forwarded.
...
- The browser UI does not directly access PostgreSQL and must use the Search API/gateway boundary.
...
2. WireGuard, admin, PostgreSQL, registry and identity credential rotation periods;
...
7. load and failure tests for WireGuard, Redpanda, PostgreSQL and Search worker jobs;
...
This document should be reviewed alongside [secret-management.md](secret-management.md), [atheros-search-privacy.md](atheros-search-privacy.md), [architecture.md](architecture.md) and [PostgreSQL manifests](../sql/postgres/). The architecture is only as safe as the least-trusted boundary and the weakest deployment control.
