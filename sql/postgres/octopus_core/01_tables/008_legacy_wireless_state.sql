-- object: octopus_core_legacy_wireless_state
-- depends_on: octopus_core_legacy_sync_ledger
-- Legacy wireless normalization tables used by the Octopus wireless
-- pipeline (WirelessProcessorSql, ProjectionSql, WirelessSql). Ported from
-- the retired TiDB 006_wireless_state / 013_projection_input_markers /
-- 014_shadow_alert_projection_run_id definitions to PostgreSQL.
...
CREATE INDEX IF NOT EXISTS devices_wg_pubkey_idx
  ON octopus_core.devices (wg_pubkey);
CREATE INDEX IF NOT EXISTS devices_last_seen_idx
  ON octopus_core.devices (last_seen);
...
CREATE INDEX IF NOT EXISTS wireless_authorized_networks_enabled_idx
  ON octopus_core.wireless_authorized_networks (enabled, location_id);
CREATE INDEX IF NOT EXISTS wireless_authorized_networks_bssid_idx
  ON octopus_core.wireless_authorized_networks (bssid);
...
CREATE INDEX IF NOT EXISTS wireless_clients_mac_idx
  ON octopus_core.wireless_clients (client_mac, last_seen);
CREATE INDEX IF NOT EXISTS wireless_clients_bssid_idx
  ON octopus_core.wireless_clients (known_bssid, last_seen);
...
CREATE TABLE IF NOT EXISTS octopus_core.wireless_shadow_alerts (
  ...
  created_at        timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at        timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  projection_run_id TEXT DEFAULT NULL,
  PRIMARY KEY (source_mac)
);

CREATE INDEX IF NOT EXISTS wireless_shadow_alerts_open_idx
  ON octopus_core.wireless_shadow_alerts (resolved_at, last_occurred_at);
...
CREATE INDEX IF NOT EXISTS wireless_frames_sensor_idx
  ON octopus_core.wireless_frames (sensor_id, observed_at);
CREATE INDEX IF NOT EXISTS wireless_frames_location_idx
  ON octopus_core.wireless_frames (location_id, observed_at);
CREATE INDEX IF NOT EXISTS wireless_frames_source_idx
  ON octopus_core.wireless_frames (source_mac, observed_at);
CREATE INDEX IF NOT EXISTS wireless_frames_bssid_idx
  ON octopus_core.wireless_frames (bssid, observed_at);
...
CREATE INDEX IF NOT EXISTS wireless_frame_network_src_idx
  ON octopus_core.wireless_frame_network (src_ip);
CREATE INDEX IF NOT EXISTS wireless_frame_network_dst_idx
  ON octopus_core.wireless_frame_network (dst_ip);
CREATE INDEX IF NOT EXISTS wireless_frame_network_app_idx
  ON octopus_core.wireless_frame_network (app_protocol);
...
CREATE INDEX IF NOT EXISTS wireless_frame_identity_session_idx
  ON octopus_core.wireless_frame_identity (session_key);
CREATE INDEX IF NOT EXISTS wireless_frame_identity_fingerprint_idx
  ON octopus_core.wireless_frame_identity (device_fingerprint);
...
CREATE INDEX IF NOT EXISTS wireless_frame_security_risk_idx
  ON octopus_core.wireless_frame_security (risk_score);
...
CREATE INDEX IF NOT EXISTS wireless_inventory_projection_inputs_time_idx
  ON octopus_core.wireless_inventory_projection_inputs (projected_at);
...
CREATE INDEX IF NOT EXISTS wireless_shadow_alert_inputs_source_idx
  ON octopus_core.wireless_shadow_alert_inputs (source_mac, projected_at);
