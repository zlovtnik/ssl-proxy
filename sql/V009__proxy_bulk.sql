-- V009: package-scoped bulk ingest for proxy telemetry.
-- Safe to rerun: table creation is guarded and package creation is replace-in-place.
...
-- ── 1b. CONNECTION_SESSIONS_CLOSE_DLQ ───────────────────────────────────────
DECLARE
  v_count INTEGER;
BEGIN
  SELECT COUNT(*) INTO v_count
...
CREATE OR REPLACE PACKAGE pkg_proxy_bulk AS
  TYPE t_varchar2_list IS TABLE OF VARCHAR2(4000);
  TYPE t_clob_list IS TABLE OF CLOB;
  TYPE t_raw_list IS TABLE OF RAW(32767);
  TYPE t_number_list IS TABLE OF NUMBER;
  TYPE t_timestamp_list IS TABLE OF TIMESTAMP;
