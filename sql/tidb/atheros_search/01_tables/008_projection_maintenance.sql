...
ALTER TABLE timing_profiles
  ...
  AFTER wall_jitter_ms;

CREATE TABLE IF NOT EXISTS sequence_transition_contributions (
  ...
  KEY sequence_transition_contributions_aggregate_idx (
    previous_token, next_token, sequence_kind
  )
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS sequence_previous_totals (
  session_key       VARCHAR(255) NOT NULL,
  previous_token    VARCHAR(191) NOT NULL,
  sequence_kind     VARCHAR(64) NOT NULL DEFAULT 'frame_sequence',
  previous_total    BIGINT NOT NULL DEFAULT 0,
  ...
  KEY sequence_previous_totals_aggregate_idx (previous_token, sequence_kind)
) ENGINE=InnoDB;
