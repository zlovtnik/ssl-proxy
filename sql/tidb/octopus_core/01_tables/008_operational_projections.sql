...
-- names preserve the Rails read contract without TiDB materialized views.
...
CREATE TABLE IF NOT EXISTS sensors (
  ...
  PRIMARY KEY (id),
  UNIQUE KEY sensors_sensor_id_uq (sensor_id),
  KEY sensors_status_seen_idx (status, last_seen_at)
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS sensor_alerts (
  ...
  updated_at  DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
  PRIMARY KEY (id),
  KEY sensor_alerts_open_idx (sensor_id, alert_type, resolved_at)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS redpanda_traffic_samples (
  id          BIGINT NOT NULL AUTO_INCREMENT,
  ...
  updated_at  DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
  PRIMARY KEY (id),
  UNIQUE KEY redpanda_traffic_samples_sample_uq (topic, sensor_id, sampled_at),
  KEY redpanda_traffic_samples_time_idx (sampled_at)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS mv_wireless_heatmap (
  location_id     VARCHAR(128) NOT NULL,
  event_count     BIGINT NOT NULL DEFAULT 0,
  ...
  KEY mv_wireless_heatmap_events_idx (event_count),
  KEY mv_wireless_heatmap_seen_idx (last_seen_at)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS v_wireless_device_inventory (
  ...
  KEY v_wireless_device_inventory_seen_idx (last_seen),
  KEY v_wireless_device_inventory_location_idx (location_id, last_seen)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS v_wireless_shadow_alerts (
  ...
  UNIQUE KEY v_wireless_shadow_alerts_dedupe_uq (dedupe_key),
  KEY v_wireless_shadow_alerts_open_idx (resolved_at, last_occurred_at)
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS v_sync_plane_health (
  projection_key                    VARCHAR(64) NOT NULL DEFAULT 'current',
  measured_at                       DATETIME(6) NOT NULL,
  wireless_events_24h_count         BIGINT NOT NULL DEFAULT 0,
  wireless_last_observed_at         DATETIME(6) DEFAULT NULL,
  ...
  last_shadow_it_alert_at           DATETIME(6) DEFAULT NULL,
  wireless_cursor_value             TEXT DEFAULT NULL,
  wireless_cursor_updated_at        DATETIME(6) DEFAULT NULL,
  projection_run_id                 CHAR(36) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,
  updated_at                        DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
