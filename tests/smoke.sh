...
wait_for_health() {
    echo "⏳ Waiting for admin liveness and BoringTun health (timeout 60s)"
    for i in {1..60}; do
        if { curl -f -s http://127.0.0.1:3002/health >/dev/null 2>&1 \
            || docker compose exec -T ssl-proxy curl -f -s http://127.0.0.1:3002/health >/dev/null 2>&1; } \
            && docker compose exec -T ssl-proxy /app/ssl-proxy boringtun show wg0 >/dev/null 2>&1; then
            echo "✅ Admin liveness and BoringTun checks passed"
            return 0
        fi
        if [ "$i" = 60 ]; then
...
assert_ready_status() {
    ...
    if [ "$ready_code" = "000" ] || [ "$ready_code" = "52" ]; then
        ready_code="$(docker compose exec -T ssl-proxy curl -s -o /tmp/smoke-ready-body.txt -w "%{http_code}" http://127.0.0.1:3002/ready || true)"
        docker compose exec -T ssl-proxy sh -lc 'cat /tmp/smoke-ready-body.txt' >"$ready_body" 2>/dev/null || true
    fi
...
assert_wg_interface() {
    ...
    echo "🔍 Checking BoringTun interface status"
    wg_output="$(docker compose exec -T ssl-proxy /app/ssl-proxy boringtun show wg0)"
    echo "$wg_output"

    if echo "$wg_output" | grep -q "interface: wg0"; then
        echo "✅ BoringTun interface is active"
    else
        echo "❌ BoringTun interface output was unexpected"
        exit 1
    fi
}
...
assert_unique_server_address() {
    ...
    echo "✅ Rendered BoringTun config contains one unique server address"
}

assert_userspace_runtime_only() {
    local tool
    for tool in wg wg-quick modprobe; do
        if docker compose exec -T ssl-proxy sh -lc "command -v ${tool} >/dev/null 2>&1"; then
            echo "❌ Unexpected legacy tooling still present in container: ${tool}"
            exit 1
        fi
    done

    if ! docker compose exec -T ssl-proxy sh -lc "command -v boringtun-cli >/dev/null 2>&1"; then
        echo "❌ Expected boringtun-cli to be present in container"
        exit 1
    fi

    echo "✅ Container contains BoringTun userspace tooling only"
}
